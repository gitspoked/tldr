...
For a repository's first full index, run `tldr peek PATH`, `tldr setup --check`,
and `tldr doctor` before `tldr init PATH`. The default Ollama models are
`mxbai-embed-large` and `qwen2.5-coder:3b-instruct`. The default embedding batch
is 32; use `TLDREADME_EMBED_BATCH_SIZE=16` when lower peak memory is needed.
...
Repository scope is also bedrock. Plans, tasks, roadmaps, and next actions stay
inside the selected repository. Cross-repository code may be used as idea
evidence only after explicit opt-in, and must not import external work items.
